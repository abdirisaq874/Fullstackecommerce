...
export default function InventoryDetailPage({ params }: { params: { sku: string } }) {
  ...
  const submit = async () => {
    ...
    await adjust({ sku: row.sku, delta: num, reason: note || reason }).unwrap();
            ...
            <div className="space-y-3">
              ...
              <Field label="Quantity change" hint="Positive to add, negative to remove">
                <div className="flex items-center gap-1.5">
                  <Button onClick={() => setDelta(d => String((Number(d) || 0) - 1))} className="!w-8 !h-9 !p-0">
              ...
              <Field label="Note (optional)">
                <Input value={note} onChange={e => setNote(e.target.value)} placeholder="PO #1234 from Aysel Tekstil" />
              </Field>

              {delta && (
                <div className="text-xs text-stone-600 bg-stone-50 p-3 rounded-md">
                  New on-hand: <strong className="text-stone-900 tabular-nums">{row.onHand + (Number(delta) || 0)}</strong>
                </div>
              )}

              <Button variant="primary" onClick={submit} disabled={adjusting || !delta || Number(delta) === 0} className="w-full">
                Apply adjustment
              </Button>
            </div>
          </Card>
